...
Class CourseTypeDAO{
	...
	public function getAllCourseTypes(){
		...
		for($i = 0; $i < sizeof($rows); $i++) {
		    ...
		    $this->_DB->doQuery("SELECT * FROM tbl_course_sessional_type where ID='".$sid."'");
	...
	public function getsessionalname($sessionalID)
	{
		...
		$this->_DB->doQuery("SELECT * FROM tbl_course_sessional_type where ID='".$sessionalID."'");
	...
	public function createCourseType($CourseType){
		...
		$SQL = "INSERT INTO tbl_course_type(ID,Name,SessionalTypeID) VALUES('$ID','$Name','$Sessional')";
	...
	public function readCourseType($CourseType){
		...
		$SQL = "SELECT * FROM tbl_course_type WHERE ID='".$CourseType->getID()."'";
	...
	public function updateCourseType($CourseType){
		...
		$SQL = "UPDATE tbl_course_type SET Name='".$CourseType->getName()."', SessionalTypeID='".$CourseType->getSessionalTypeID()."' WHERE ID='".$CourseType->getID()."'";
	...
	public function deleteCourseType($CourseType){
		...
		$SQL = "DELETE from tbl_course_type where ID ='".$CourseType->getID()."'";
...
echo '<br> log:: exit the class.CourseTypedao.php';
